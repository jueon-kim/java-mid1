...
## 기본형의 한계 
- 자바는 객체 지향 언어 이다.
- `int, double` 같은 `기본형(primitive Type)`은 객체가 아니다.
- 기본형은 객체가 아니기 때문에 메서드를 제공할 수 없다.
- 기본형 데이터 타입은 `null` 값을 가질 수 없다. 때로는 데이터가 `없음` 이라는 상태를 나 타내야 할 필요가 있는데, 기본형은 항상 값을 가지기 때문에 이런 표현을 할 수 없다.


## 기본형의 한계 2 
- 기본형은 항상 값을 가져야 한다. 
- 데이터가 `없음` 이라는 상태가 필요할 수 있다. 

## 래퍼 클래스 
- 기본형을 객체로 감싸서 더 편리하게 사용하다록 도와주기 때문에 상당히 유용하다 
-  불변이다. 
- `equals`로 배교 해야 한다. 
...
***래퍼 클래스 생성 - 박싱 (Boxing)***
- 기본형을 래퍼 클래스로 변경하는 것을 마치 밗스에 물건을 넣은 것 같다고 해서 박생(Boxing) dlfk gksek. 
- `new Integer(10)`은 직접 사용하면 안된다
- `new Integer(10)`을 사용해서 객체를 생성하고 돌려준다.
- `Integer.valueOf()` 에는 성능 최적화 기능이 있다. -128 ~ 127범위의 `Ìnteger`클래스를 미리 생성해준다. 


***int Value() - 언박생(Unboxing)
- 래퍼 클래스에 들어 있는 기본형 값을 다시 꺼내는 매서디이다. 
- 박스에 들어있는 물건을 꺼내는 것 같다고 해서 언박싱이라 한다. 

***비교는 equals() 사용*** 
- 래퍼 클래스는 객체이기 떄문에 `==` 비교를 하면 인스턴스의 참조값을 비교 한다. 
...
- 기본형은 메모리에서 단순히 그 크기만큼의 공간을 차지한다. 예를 들어 `int` 는 보통 4바이트의 메모리를 사용한 다. 
- 래퍼 클래스의 인스턴스는 내부에 필드로 가지고 있는 기본형의 값 뿐만 아니라 자바에서 객체 자체를 다루는데 
- 필요한 객체 메타데이터를 포함하므로 더 많은 메모리를 사용한다. 자바 버전과 시스템마다 다르지만 대략 8~16 바이트의 메모리를 추가로 사용한다.
...
- 상수를 사용 해야 한다 하지만 근본적으로 해결 할수 없다 String타입은 어떤 문자열을 막을수 방법이 없다. 


### 타입 안전 열거형 패턴 
- 여기서 영어인 `enum` 은 `enumeration` 의 줄임말인데, 번역하면 열거라는 뜻이고, 어떤 항목을 나열하는 것을 뜻한
  다
- 나열한 항목이 아닌 것은 사용할수 없다. String 처럼 아무 문자열이나 사용할수 없다. 


# 중첩 클래스, 내부 클래스 
- 중첨(Nested): 어떤 다른 것이 내부에 위치하거나 포함되는 구조적인 관계
...
- 인스턴스의 차이 
- 정적 중첩 클래스는 바깥 클래스와 전혀 다른 클래스이다. 따라서 바깥 클래스의 인스턴스에 소속되짖 않는다. 
- 내부 클래스는 바깥 클래스를 구성하는 요소이다. 따라서 바깥 클래스의 인스텉스에 소속된다. 
...
이 코드와 정적 중첩 클래스의 유일한 차이는 같은 클래스에 있으니 `private` 접근 제어자에 접근할 수 있다는 정도이
다.

# 중첩 클래스, 내부 클래스 
단순한 문제였지만 프로그래밍에서 중복을 제거하고, 좋은 코드를 유지하는 핵심은 변하는 부분과 변하지 않는 부분을
분리하는 것이다. 여기서는 변하지 않는 "프로그램 시작", "프로그램 종료"를 출력하는 부분은 그대로 유지하고, 상황에
따라 변화가 필요한 문자열은 외부에서 전달 받아서 처리했다.
